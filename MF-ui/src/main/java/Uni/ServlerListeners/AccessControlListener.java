...
public class AccessControlListener implements HttpSessionListener {
    ...
    public void sessionCreated(HttpSessionEvent se) {
        ...
        Logger.getLogger("test").log(Level.INFO, "MF - Http Session {0} - Created.",
                se.getSession().getId());
        Logger.getLogger("test").log(Level.INFO, "MF - Shiro Sessions : {0}", shiroAuthObject.getUsersSessions().toString());
    }

    @Override
    public void sessionDestroyed(HttpSessionEvent se) {
        Logger.getLogger("test").log(Level.INFO, "MF - Session  - Destroyed.");
        Logger.getLogger("test").log(Level.INFO, "MF - Shiro Sessions : {0}", shiroAuthObject.getUsersSessions().toString());
